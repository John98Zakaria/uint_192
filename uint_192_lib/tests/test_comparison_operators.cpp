...
SCENARIO("Test < operator", "[uint_192]") {
    ...
    GIVEN("A uint_192 of value {0,1,0}") {
        ...
        THEN("It not smaller than another uint_192 of value {0,0,0}") {
            uint192_lib::uint_192 b{0};
            REQUIRE_FALSE(a < b);
        }
    }
...
SCENARIO("Test > operator", "[uint_192]") {
    GIVEN("A uint_192 of value {5,0,0}") {
        ...
        THEN("It not larger than another uint_192 of value {6,0,0}") {
            uint192_lib::uint_192 b{6};
            REQUIRE_FALSE(a > b);
        }

        THEN("It not larger than another uint_192 of value {0,1,0}") {
            uint192_lib::uint_192 b{0, 1};
            REQUIRE_FALSE(a > b);
        }

        THEN("It not larger than another uint_192 of value {0,0,1}") {
            uint192_lib::uint_192 b{0, 0, 1};
            REQUIRE_FALSE(a > b);
        }

        THEN("It not larger than another uint_192 of value {0,0,0}") {
            uint192_lib::uint_192 b{0};
            REQUIRE(a > b);
        }
    ...
    GIVEN("A uint_192 of value {0,1,0}") {
        ...
        THEN("It not larger than another uint_192 of value {0,2,0}") {
            uint192_lib::uint_192 b{0, 2};
            REQUIRE_FALSE(a > b);
        }
        THEN("It not larger than another uint_192 of value {0,0,1}") {
    ...
    GIVEN("A uint_192 of value {0,0,1}") {
        ...
        THEN("It not larger than than another uint_192 of value {0,0,2}") {
            uint192_lib::uint_192 b{0, 0, 2};
            REQUIRE_FALSE(a > b);
        }

        THEN("It not larger than than another uint_192 of value {0,0,3}") {
            uint192_lib::uint_192 b{0, 0, 3};
            REQUIRE_FALSE(a > b);
        }

        THEN("It not larger than than another uint_192 of value {0,0,UINT64_MAX}") {
            uint192_lib::uint_192 b{0, 0, UINT64_MAX};
            REQUIRE_FALSE(a > b);
        }
        ...
        THEN("It is larger than another uint_192 of value {UINT64_MAX,0,0}") {
            uint192_lib::uint_192 b{UINT64_MAX, 0, 0};
            REQUIRE(a > b);
        }

        THEN("It is larger than another uint_192 of value {UINT64_MAX,UINT64_MAX,0}") {
            uint192_lib::uint_192 b{UINT64_MAX, UINT64_MAX, 0};
            REQUIRE(a > b);
        }
